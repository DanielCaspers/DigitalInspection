...
{
    internal class MurphyAutomotiveOAuth2AuthenticationHandler : AuthenticationHandler<MurphyAutomotiveOAuth2AuthenticationOptions>
    {
        private readonly ILogger _logger;
        private readonly HttpClient _httpClient;

        public MurphyAutomotiveOAuth2AuthenticationHandler(HttpClient httpClient, ILogger logger)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
				...
				// TODO DJC FIXME CUSTOMIZE FOR AUTHORIZE ENDPOINT
                ...
                {
                    context.Identity.AddClaim(new Claim(ClaimTypes.GivenName, context.LastName, 
                        ClaimValueTypes.String, Options.AuthenticationType));
                }
                if (!string.IsNullOrEmpty(context.EmployeeId))
                {
                    context.Identity.AddClaim(new Claim(ClaimTypes.Actor, context.EmployeeId,
                        ClaimValueTypes.String, Options.AuthenticationType));
                }                
                if (!string.IsNullOrEmpty(context.FirstName))
                {
                    context.Identity.AddClaim(new Claim(ClaimTypes.Name, context.FirstName, ClaimValueTypes.String,
                        Options.AuthenticationType));
                }
                if (!string.IsNullOrEmpty(context.UserImage))
                {
                    context.Identity.AddClaim(new Claim(ClaimTypes.UserData, context.UserImage, ClaimValueTypes.String, 
                        Options.AuthenticationType));
                }
        ...
        protected override Task ApplyResponseChallengeAsync()
        {
            ...
            if (challenge != null)
            {
                ...
                queryStrings.Add("redirectURL", redirectUri);

                // space separated
                //string scope = string.Join(" ", Options.Scope);
                //if (string.IsNullOrEmpty(scope))
                //{
                //    // MurphyAutomotive OAuth 2.0 asks for non-empty scope. If user didn't set it, set default scope to 
                ...
                var redirectContext = new MurphyAutomotiveOAuth2ApplyRedirectContext(
                    Context, Options,
        ...
        private async Task<bool> InvokeReplyPathAsync()
        {
            if (Options.CallbackPath.HasValue && Options.CallbackPath == Request.Path)
            {
                ...
                var context = new MurphyAutomotiveOAuth2ReturnEndpointContext(Context, ticket);
